...
func Gen(w io.Writer, x interface{}, c *Config) error {
	...
	firstChar := []rune(strings.ToLower(c.TypeName))[0]
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if field.Type.Kind() == reflect.Bool {
			...
			print(buf, "\treturn %s((%c >> %d) & %#x)", retType, firstChar, shift, mask)
		}
		print(buf, "}")
		...
		print(buf, "\nfunc (%c %s) Set%s(v %s) %s {", firstChar, c.TypeName, f.name, retType, c.TypeName)
		if field.Type.Kind() == reflect.Bool {
			print(buf, "\tconst bit = 1 << %d", shift)
			print(buf, "\tif v {")
			print(buf, "\t\treturn %c | bit", firstChar)
			print(buf, "\t}")
			print(buf, "\treturn %c & ^%s(bit)", firstChar, c.TypeName)
		} else {
			print(buf, "\t%c &= ^%s(%#x << %d)", firstChar, c.TypeName, mask, shift)
			print(buf, "\t%c |= (%s(v) & %#x) << %d", firstChar, c.TypeName, mask, shift)
			print(buf, "\treturn %c", firstChar)
		}
		print(buf, "}")
	}

	if c.Package != "" {
		print(w, "// Code generated by github.com/rj45/rj32/emurj/bitfield. DO NOT EDIT.\n")
		print(w, "package %s\n", c.Package)
	}
